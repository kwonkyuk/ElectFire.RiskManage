...
// Check and handle specific API keys and environmental errors
function handleApiError(error: any, res: any, context: string) {
  console.error(`[Gemini Proxy Error - ${context}]:`, error);
  const errorMessage = String(error.message || "").toLowerCase();
  const errorStatus = error.status ? Number(error.status) : 0;
  ...
  if (!process.env.GEMINI_API_KEY) {
    return res.status(401).json({
      error: "설정 오류: GEMINI_API_KEY 환경 변수가 배포 서버에 설정되지 않았습니다. Vercel Project Settings -> Environment Variables 메뉴에서 GEMINI_API_KEY 값을 등록한 후 재배포해 주십시오."
    });
  }

  // 2. Invalid API Key
  if (errorMessage.includes("api_key_invalid") || errorMessage.includes("invalid key") || errorMessage.includes("key not found") || (errorStatus === 400 && errorMessage.includes("key"))) {
    return res.status(401).json({
      error: "인증 오류: 등록된 GEMINI_API_KEY가 유효하지 않습니다. 구글 AI 스튜디오(Google AI Studio)에서 올바른 API 키를 생성하여 등록해 주십시오."
    });
  }

  // 3. Quota/Billing Error
  if (errorMessage.includes("quota") || errorMessage.includes("billing") || errorMessage.includes("limit") || errorMessage.includes("exhausted")) {
    return res.status(403).json({
      error: "할당량 초과: Gemini API의 사용 한계(Quota)를 초과하였거나 결제 상태를 확인해야 합니다. 잠시 후 재시도하거나 구글 AI 스튜디오 설정을 확인해 주십시오."
    });
  }

  // 4. Default Congestion / Transient Network Error
  return res.status(503).json({
    error: "AI 튜터 통신 서버가 붐비고 있네요. 잠시(2~3초) 후에 다시 시도해 주세요!"
  });
}
...
${contextTopic ? `현재 학생은 다음 주차 내용을 심화 학습하고 있으니 참고하여 답하라: ${contextTopic}` : ""}`;
    ...
    return handleApiError(error, res, "Chat Proxy");
  }
});
...
JSON Schema 형식에 맞추어 다음 구조로만 정확하게 응답하라. 다른 부가 설명 텍스트는 절대 허용하지 않는다.`;
    ...
    return handleApiError(error, res, "Quiz Generator");
  }
});
...
한국어로 친근하고 전문적인 소견 보고서 형식으로 작성해 주라.`;
    ...
    return handleApiError(error, res, "Simulation Proxy");
  }
});
...
우아하고 격식 있는 한국어로 단락을 나누어 친절히 설명하라.`
    ...
    return handleApiError(error, res, "Vision Proxy");
  }
});
