...
app.post("/api/chat", async (req, res) => {
  try {
    ...
    const systemInstruction = `너는 동원과학기술대학교 산업안전관리과의 '전기화재 위험관리' 전공 주임교수이자, 30년 현장 경력의 화재 감식학 권위자 'YUKWON' 교수이다. 
...
${contextTopic ? `현재 학생은 다음 주차 내용을 심화 학습하고 있으니 참고하여 답하라: ${contextTopic}` : ""}`;
    ...
    // Simplified generateContent query as single turn structured query
    ...
    res.status(500).json({ error: error.message || "AI Tutor server error" });
  }
});
...
JSON Schema 형식에 맞추어 다음 구조로만 정확하게 응답하라. 다른 부가 설명 텍스트는 절대 허용하지 않는다.`;
    ...
    res.status(500).json({ error: error.message || "AI Quiz generator error" });
  }
});
...
한국어로 친근하고 전문적인 소견 보고서 형식으로 작성해 주라.`;
    ...
    res.status(500).json({ error: error.message || "AI Simulator server error" });
  }
});

// 5. AI Forensics Optical & Metallurgical Image Analyser (Vision)
app.post("/api/vision-detect", async (asyncReq, asyncRes) => {
  try {
    const { fileData, mimeType, description } = asyncReq.body;
    if (!fileData || !mimeType) {
      return asyncRes.status(400).json({ error: "Base64 fileData and standard mimeType is required." });
    }
...
우아하고 격식 있는 한국어로 단락을 나누어 친절히 설명하라.`
    ...
    asyncRes.json({ analysis: response.text });
  } catch (error: any) {
    console.error("Vision Proxy Error:", error);
    asyncRes.status(500).json({ error: error.message || "AI Vision Analyser server error" });
  }
});
...
// 6. Vite middleware integration for live development reloading / serving production static files
async function startServer() {
  ...
  app.listen(PORT, "0.0.0.0", () => {
    console.log(`[DST Platform Server] running on http://0.0.0.0:${PORT}`);
  });
}
